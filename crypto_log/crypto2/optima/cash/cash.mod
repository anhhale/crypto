...
% Declare variables
% Price variables
var  Y C Pi N m    D_star   totutility1
 mc                
    Z         
    w         
    R         
...
//Bank variables
    ...
    upsilon   
    mu        
    mu_star  utility %Exogenous processes
...
leverage= log(phi);
TB =EX-M;
utility=log(C-zeta_0/(1+zeta)*L^(1+zeta))-chi_dc*(Gamma-trs)^2 ;
totutility1  = log(C-zeta_0/(1+zeta)*L^(1+zeta))-chi_dc*(Gamma-trs)^2  + betta*totutility1(+1);
%logcryp=log(cryp);
end;
...
DC=0;
Lambda1=Lambda_ss;
cryp = 0;%((Lambda1-betta*Lambda1*R_c/Pi)/mu_c)^(1/(-rho_cryp));
x_c=cryp/K_b;
theta_c=1;
