warning off
gridPI=1.49:0.01:2.5;    % grid for phipi
gridE=0.26:0.01:0.8; % grid for phiz

phipi=gridPI;  phiee=gridE;
welfmean=-1e8*ones(length(phipi),length(phiee)); % Matrix of welfare values for each parameter combinations
N=(length(phipi)*length(phiee));                 % Number of combinations
// Declare variables
// Price variables
var Welf
...
// Additional params
            ...
            midc
            Gamma
            chi_dc phipi_dc phiy_dc phie 
trs_ss m_ss DC_ss Lambda_ss gam_ss css hss
;
...
%phipi_dc=1.49;
 %phiy_dc=0.26;
 css=1.944886671710838;
hss=0.262150677255713;
      theta = 0.401;
            theta_0 = 0.401; %0.399;
            sigma = 0.94;
            xi = 0.0045; %0.0046;
...
for kk=1:length(phipi)
    phipi_dc=phipi(kk);
for ww=1:length(phiee)
    phiy_dc=phiee(ww) ;%phiy(ww)
...
ln(R_dc)=(1-0.92)*ln(R_dc(-1)) +0.92*(phipi_dc*ln(Pi) + phiy_dc*ln(Y/steady_state(Y)));%flex
...
log(R/R_ss) = (1-rho_i)*(omega_pi*log(Pi/1)+ 0*log(Y/steady_state(Y))+ phie*log(DE)) + rho_i*log(R(-1)/(R_ss))+ varepsilon_R  ;
...
DE=1;
%c_sup=c_share*Y;
Welf= log(C-  zeta_0/(1+zeta)*L^(1+zeta))/(1-betta);  
sigma_Rc=ln(1);
logY =log(Y);
logPi = log(Pi);
logR = log(R);
...
welfmean(kk,ww)=oo_.dr.ys(1)+0.5*oo_.dr.ghs2(oo_.dr.inv_order_var(1)); % welfare value
cc=cc+1;                                                                           % counter
...
phipi_opt=phipi(dd)                                          
phiy_opt=phiee(ee)                                                                                        
gain = cons_equiv(welf_opt,welfmean(1,1),css,hss,zeta,zeta_0,betta) % consumption gain
