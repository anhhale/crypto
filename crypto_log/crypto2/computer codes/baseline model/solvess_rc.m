function [lambda, c, h, inv, y, k, ne, atilde,b , nc, Hc, bc, Fc, vc, H1c, Gc, omegacbar, rc, G1c,  ns, Hs, bs, Fs, vs, H1s, Gs, omegasbar, rs, G1s, dc, kb, rcl, w, rp,  q,  rk, a, vk, gdp, sigma, utility, totutility, fshock, ai] = solvess_rc(BETA,DELTA,ALPHA,THETAE,THETA,RCTARGET,SIGMA,THETAC,VK,GAMMAC,GAMMAS,KAPPAB,CHI,FRISCH,HABIT )
...
h = ((1-THETA)*ALPHA*(1/atilde)*lambdaxy /CHI)^(1/((FRISCH+1)));
y = (a*(k_y)^(1-ALPHA)*h^((1-THETA)*ALPHA))^(1/(ALPHA));
k = k_y*y;
c = c_y*y;
inv = inv_y*y;
%lambda = 1/c;
lambda = 1/(c-HABIT*c)-BETA*HABIT/(c-HABIT*c);
...
utility = log(( c-HABIT*c )) +PSID*log(dc) - CHI*h^(FRISCH+1)/(FRISCH+1);
