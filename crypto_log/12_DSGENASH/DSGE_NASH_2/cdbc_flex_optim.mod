...
// Declare variables
// Price variables
var  Y C Pi N    DC cryp m D_star   Welf Recursive_natural_welfare_equivalent Welfare_gap
 mc                
    Z         
    w         
    R         
...
//Specify parameters
//Banks
parameters  theta       lambda_utility
            theta_0     
            sigma       
            xi          
            varkappa_b  
...
// Additional params
            ...
            Gamma
            chi_dc
trs_ss m_ss DC_ss Lambda_ss gam_ss opt_pi opt_y opt_ex rho_r2
;
...
set_param_value('c_share',c_share);
           opt_pi= 1.49;
 opt_y= 0.26; opt_ex=0.3;
      theta = 0.401;rho_r2= 0.92;
            theta_0 = 0.401; %0.399;
            sigma = 0.94;
            xi = 0.0045; %0.0046;
...
load param_flex
set_param_value('rho_i',0.38498);
set_param_value('omega_pi',5);
set_param_value('opt_ex',0.27265);
model;
...
ln(R_dc)=(1-rho_r2)*ln(R_dc(-1)) +rho_r2*(opt_pi*ln(Pi) + opt_y*ln(Y/steady_state(Y)));%flex
...
log(R/R_ss) = (1-rho_i)*(omega_pi*log(Pi/1)+ 0.*log(Y/steady_state(Y))+ opt_ex*log(DE)) + rho_i*log(R(-1)/(R_ss))+ varepsilon_R  ;%(1-rho_i)*
...
logN = log(N);
spread =((Z(+1)+lambda*Q(+1))/Q-R/Pi(+1));
logDC =log(DC);
logm =log(m);
leverage= log(phi);
TB =EX-M;
...
%%%%welfare
utility=log(C-zeta_0/(1+zeta)*L^(1+zeta));
Welf  = log(C-zeta_0/(1+zeta)*L^(1+zeta)) + betta*Welf(+1);
Recursive_natural_welfare_equivalent= (log((1-lambda_utility)*C-zeta_0/(1+zeta)*L^(1+zeta))) +betta*Recursive_natural_welfare_equivalent(+1);
Welfare_gap=Welf-Recursive_natural_welfare_equivalent;
end;
...
logcryp=log(cryp);
utility=log(C-zeta_0/(1+zeta)*L^(1+zeta));
Welf=utility/(1-betta);
Recursive_natural_welfare_equivalent= 1/(1-betta)*(log((1-lambda_utility)*C-zeta_0/(1+zeta)*L^(1+zeta)));
Welfare_gap=Welf-Recursive_natural_welfare_equivalent;
end ;
resid;
steady;
...
stoch_simul(nograph,noprint,pruning,order=2,AR=0,IRF=0);
