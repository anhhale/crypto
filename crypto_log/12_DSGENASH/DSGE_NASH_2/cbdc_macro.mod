...
// Declare variables
// Price variables
var  Y C Pi N    DC cryp m D_star   Welf
 mc                
    Z         
    w         
    R         
...
// Additional params
            ...
            Gamma
            chi_dc
trs_ss m_ss DC_ss Lambda_ss gam_ss opt_pi opt_y opt_ex rho_r2 opt_star opt_c
;
...
%set_param_value('rho_i',rho_i);
%set_param_value('opt_ex',opt_ex);
%set_param_value('opt_star',opt_star);
%set_param_value('opt_c',opt_c);

opt_star=0.1; opt_c=0.1;
           opt_pi= 1.49;
...
mu_cryp = Omega*((1-tau_c)*R/Pi(+1)-(R_c/Pi(+1)));
[name='cost advantage of foreign currency debt over home deposits, eq. (18)']
mu_star = Omega*((1-tau_star)*R/Pi(+1)-epsilon(+1)/epsilon*R_star);
...
log(R/R_ss) = (omega_pi*log(Pi/1)+ 0.*log(Y/steady_state(Y))+ opt_ex*log(DE)) + rho_i*log(R(-1)/(R_ss))+ varepsilon_R  ;%(1-rho_i)*
...
tau_c=opt_c*(ln(K_b)-ln(steady_state(K_b)));
tau_star=opt_star*(ln(K_b)-ln(steady_state(K_b)));
//Observed variables
...
%%%%welfare
utility=log(C-zeta_0/(1+zeta)*L^(1+zeta))-chi_dc*(Gamma-trs)^2 +mu_c*(cryp^(1-rho_cryp))/(1-rho_cryp);
Welf  = log(C-zeta_0/(1+zeta)*L^(1+zeta))-chi_dc*(Gamma-trs)^2 +mu_c*(cryp^(1-rho_cryp))/(1-rho_cryp) + betta*Welf(+1);
end;
