...
// Declare variables
// Price variables
var totutility1 Y C  I Pi N  DC cryp m D_star   
 mc                
    Z         
    w         
    R         
...
// Additional params
            ...
            Gamma
            chi_dc
trs_ss m_ss DC_ss Lambda_ss gam_ss opt_pi opt_y opt_ex
;
...
%ln(R_dc)=(1-0.92)*ln(R_dc(-1)) +0.92*(1.49*ln(Pi) + 0.26*ln(Y/steady_state(Y)));%flex
...
gam - chi_dc*(2*(Gamma - trs)*(trs/(m+DC) - 1/(m+DC))) - Lambda1 + betta*Lambda1(+1)*ep_m/Pi(+1);
[name='FOC wrt crypto']
...
log(R/R_ss) = (1-rho_i)*(omega_pi*log(Pi/1)+ 0.*log(Y/steady_state(Y))+ opt_ex*log(DE)) + rho_i*log(R(-1)/(R_ss))+ varepsilon_R  ;
...
totutility1  = log(C-zeta_0/(1+zeta)*L^(1+zeta))-chi_dc*(Gamma-trs)^2 +mu_c*(cryp^(1-rho_cryp))/(1-rho_cryp) + betta*totutility1(+1);
...
stoch_simul(order=2,irf=40,noprint, nograph);
