...
C = C2Y*Y;
psi = Theta*phi;
Lambda_ss=1/(C-zeta_0*L^(1+zeta)/(1+zeta));
m=((Lambda_ss-betta*Lambda_ss*ep_m)/mu_m)^(1/(-rho_m));
...
D = Q*Kb*(1+varkappa_b/2*x^2) - N - eps*Dstar -cryp;
...
xi=(N - (sigma*((Z+lambda*Q)*Kb-D*R/Pi -cryp*1/Pi -upsilon*Rstar*Dstar)))/((Z+lambda*Q)*K);
...
F = @(x) [((C)^(rho_dc))*mi*x(4)*x(1)^(-rho_dc) - chi_dc*(2*(Gamma - x(2))*(x(2)/(x(1)+0) - 1/(x(1)+0))) - x(3) + betta*x(3)*ep_m/Pi;
          x(2) - x(1)/(x(1)+0);
          x(3) + x(4) - 1/(C-zeta_0*L^(1+zeta)/(1+zeta));%/(C-zeta_0*L^(1+zeta)/(1+zeta))
...
F = @(x) [x(3) - x(2) + betta*x(2)*ep_m/Pi;
         1/(C-zeta_0*L^(1+zeta)/(1+zeta))-x(2)-x(3);
          C-x(1)];  
%initial value for x(2) guessed at 0.5
