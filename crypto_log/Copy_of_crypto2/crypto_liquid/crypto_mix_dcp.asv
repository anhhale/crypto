...
Lambda1
//DCP
p_HH, p_UH, p_RH, p_HU, p_HR, 
p_U_UH, p_H_UH, p_U_HU, p_H_HU, 
p_H_HR, p_U_HR, p_R_HR, p_H_RH, p_U_RH, p_R_RH, 
p_U_UH_reset, p_H_UH_reset, p_U_HU_reset, p_H_HU_reset, 
p_H_HR_reset, p_U_HR_reset, p_R_HR_reset, p_H_RH_reset, p_U_RH_reset, p_R_RH_reset,p_HH_reset, i_U
B_U bfl
y_HH, y_HU, y_HR,y_UH,y_RH,
y_U_UH, y_H_UH, y_U_HU, y_H_HU,
y_U_HR, y_H_HR, y_R_HR, y_U_RH, y_H_RH, y_R_RH,X
   ;
...
Ynet = Y - epsilon*M - kappa/2*(Pi-1)^2*Y - chi_h - chi_b;
 //DCP
%%%%pricing DCP
Y =exp(y_HH)+   exp(y_HU) + exp(y_HR) + exp(i)+ exp(g)+  ;%55
exp(y_HU) = theta_U_HU*exp(y_U_HU) + theta_H_HU*exp(y_H_HU);
exp(y_HR) = theta_U_HR*exp(y_U_HR) + theta_H_HR*exp(y_H_HR) + theta_R_HR*exp(y_R_HR);
exp(y_UH) = theta_U_UH*exp(y_U_UH) + theta_H_UH*exp(y_H_UH);
exp(y_RH) = theta_U_RH*exp(y_U_RH) + theta_H_RH*exp(y_H_RH) + theta_R_RH*exp(y_R_RH);
exp(y_HH) = gamma_H*(1-varepsilon*p_HH)^(sigma/varepsilon)*(exp(c) + exp(x))   ; %55 + exp(as)*(exp(ns(-1)-pi)/G+exp(bs))*MS*K/(K-1)*(OMEGAM - OMEGAM^K*exp(omegas)^(-K+1)) + exp(ap)*(exp(np(-1)-pi)/G+exp(bp))*MP*K/(K-1)*(OMEGAM - OMEGAM^K*exp(omegap)^(-K+1))
exp(y_U_UH) = gamma_U*(1-varepsilon*(p_U_UH+e_U))^(sigma/varepsilon)*(exp(c)+exp(x));
exp(y_H_UH) = gamma_U*(1-varepsilon*(p_H_UH))^(sigma/varepsilon)*(exp(c)+exp(x));
exp(y_H_RH) = gamma_R*(1-varepsilon*(p_H_RH))^(sigma/varepsilon)*(exp(c)+exp(x));
exp(y_U_RH) = gamma_R*(1-varepsilon*(p_U_RH+e_U))^(sigma/varepsilon)*(exp(c)+exp(x));
exp(y_R_RH) = gamma_R*(1-varepsilon*(p_R_RH+e_R))^(sigma/varepsilon)*(exp(c)+exp(x));%65
exp(y_U_HU) = (1-varepsilon*(p_U_HU-p_star_U))^(sigma/varepsilon)*(exp(wd_U));
exp(y_H_HU) = (1-varepsilon*(p_H_HU-e_U-p_star_U))^(sigma/varepsilon)*(exp(wd_U));
exp(y_H_HR) = (1-varepsilon*(p_H_HR-e_R-p_star_R))^(sigma/varepsilon)*(exp(wd_R));
exp(y_U_HR) = (1-varepsilon*(p_U_HR-e_R+e_U-p_star_R))^(sigma/varepsilon)*(exp(wd_R));
exp(y_R_HR) = (1-varepsilon*(p_R_HR-p_star_R))^(sigma/varepsilon)*(exp(wd_R));

// Aggregate prices
gamma_H*p_HH + gamma_U*p_UH + gamma_R*p_RH = 0;          // 
p_UH = theta_U_UH*(p_U_UH+e_U) + theta_H_UH*p_H_UH;     // Aggregate price of imports from US
p_HU = theta_U_HU*(p_U_HU+e_U) + theta_H_HU*p_H_HU;     // Aggregate price of exports to US
p_RH = theta_U_RH*(p_U_RH+e_U) + theta_H_RH*p_H_RH + theta_R_RH*(p_R_RH + e_R); // Aggregate price of imports from ROW
p_HR = theta_U_HR*(p_U_HR+e_U) + theta_H_HR*p_H_HR + theta_R_HR*(p_R_HR + e_R); // Aggregate price of exports to ROW

// Price reset equations
p_U_UH_reset = BETA*theta_p*p_U_UH_reset(+1) + (1-BETA*theta_p)/(1+Gamma)*(mc_U_U + Gamma*(-e_U) + mu_bar);
p_H_UH_reset = BETA*theta_p*(p_H_UH_reset(+1) + pi(+1)) + (1-BETA*theta_p)/(1+Gamma)*(mc_U_U + e_U + mu_bar);
p_U_HU_reset = BETA*theta_p*(p_U_HU_reset(+1)) + (1-BETA*theta_p)/(1+Gamma)*(mc - e_U + Gamma*p_star_U+ mu_bar);
p_H_HU_reset = BETA*theta_p*(p_H_HU_reset(+1)+pi(+1)) + (1-BETA*theta_p)/(1+Gamma)*(mc + Gamma*(p_star_U+e_U) + mu_bar);
p_U_RH_reset = BETA*theta_p*(p_U_RH_reset(+1)) + (1-BETA*theta_p)/(1+Gamma)*(mc_R_R - e_U + e_R + Gamma*(-e_U)+ mu_bar);
p_H_RH_reset = BETA*theta_p*(p_H_RH_reset(+1) + pi(+1)) + (1-BETA*theta_p)/(1+Gamma)*(mc_R_R + e_R + mu_bar);
p_R_RH_reset = BETA*theta_p*(p_R_RH_reset(+1)) + (1-BETA*theta_p)/(1+Gamma)*(mc_R_R + Gamma*(-e_R)+ mu_bar);
p_U_HR_reset = BETA*theta_p*(p_U_HR_reset(+1)) + (1-BETA*theta_p)/(1+Gamma)*(mc - e_U + Gamma*(p_star_R- e_U + e_R)+ mu_bar);
p_H_HR_reset = BETA*theta_p*(p_H_HR_reset(+1) + pi(+1)) + (1-BETA*theta_p)/(1+Gamma)*(mc + Gamma*(p_star_R+e_R) + mu_bar);
p_R_HR_reset = BETA*theta_p*(p_R_HR_reset(+1)) + (1-BETA*theta_p)/(1+Gamma)*(mc - e_R + Gamma*(p_star_R)+ mu_bar);
p_HH_reset = BETA*theta_p*(p_HH_reset(+1)+pi(+1)) + (1-BETA*theta_p)/(1+Gamma)*(mc + mu_bar);

// Price evolution
p_U_HU - p_U_HU(-1) = (1-theta_p)*(p_U_HU_reset - p_U_HU(-1));
p_H_HU - p_H_HU(-1)+pi = (1-theta_p)*(p_H_HU_reset - p_H_HU(-1)+pi);
p_U_HR - p_U_HR(-1) = (1-theta_p)*(p_U_HR_reset - p_U_HR(-1));
p_H_HR - p_H_HR(-1)+pi = (1-theta_p)*(p_H_HR_reset - p_H_HR(-1)+pi);
p_R_HR - p_R_HR(-1) = (1-theta_p)*(p_R_HR_reset - p_R_HR(-1));
p_U_UH - p_U_UH(-1) = (1-theta_p)*(p_U_UH_reset - p_U_UH(-1));
p_H_UH - p_H_UH(-1)+pi = (1-theta_p)*(p_H_UH_reset - p_H_UH(-1)+pi);
p_U_RH - p_U_RH(-1) = (1-theta_p)*(p_U_RH_reset - p_U_RH(-1));
p_H_RH - p_H_RH(-1)+pi = (1-theta_p)*(p_H_RH_reset - p_H_RH(-1)+pi);
p_R_RH - p_R_RH(-1) = (1-theta_p)*(p_R_RH_reset - p_R_RH(-1));
p_HH - p_HH(-1)+pi     = (1-theta_p)*(p_HH_reset - p_HH(-1)+pi);
