...
myfun = @(s,sigma1,betta,xi,theta) (1-sigma1)*(betta*(s+(sstar^2/(2*varkappa_b)))+xi*((1+s)/(1-(s/varkappa_h))))*(sigma1*(s+(sstar^2/(2*varkappa_b)))+xi*((1+s)/(1-(s/varkappa_h)))) - theta*(betta-sigma1)*(sigma1*(1-betta)*(s+(sstar^2/(2*varkappa_b)))+(1-sigma1)*xi*((1+s)/(1-(s/varkappa_h))));  % parameterized function
%c = 2;                    % parameter
fun = @(s) myfun(s,sigma1,betta,xi,theta);    % function of x alone
x0 = [0.00 1];
s = fzero(fun,x0);
%Z=0.040142257852393;
%s=betta*(Z+lambda) -1
...
%function f = fun(n)
 %   f = (1-sigma1)*(betta*s+xi*(1+s))*(sigma1*s+xi(1+s))-theta*(betta-sigma1)*(sigma1*(1-betta)*s+(1-sigma1)*xi*(1+s));
%end

%fun = (1-sigma1)*(betta*s+xi*(1+s))*(sigma1*s+xi(1+s))-theta*(betta-sigma1)*(sigma1*(1-betta)*s+(1-sigma1)*xi*(1+s));
%x0 = [0.000001 1];
%swork = fzero('fun',x0,options);
%s = swork;

%myfun = @(s,sigma1,betta,xi,theta) (1-sigma1)*(betta*s+xi*(1+s))*(sigma1*s+xi(1+s))-theta*(betta-sigma1)*(sigma1*(1-betta)*s+(1-sigma1)*xi*(1+s));  % parameterized function
%c = 2;                    % parameter
%fun1 = @(s) myfun(s,sigma1,betta,xi,theta);    % function of x alone
%swork = fzero(fun1,[0.0001 1])
cryp = 1.0274;

%s = swork
x= 0.189349;
%(L/(1-alphha_K-alphha_M))^(1-alphha_K-alphha_M)
phi=4;
%Here is missed code FINISH
Z = (s+1)/betta-lambda; %6
Y = ((1-1/eta)^(1+zeta*(alpha_K+alpha_M))*(A/(eps^alpha_M*Z^alpha_K))^(1+zeta))^(1/(zeta*(1-alpha_K-alpha_M)))/((1-alpha_K-alpha_M)*(2.0568*zeta0)^(1/zeta)); %9
...
%phi = (betta-sigma1)/(sigma1*(s+sstar*x)+xi*(1+s)); %13
...
%N = sigma1*((Z+lambda*Q)*Kb-cryp-D*R/Pi-eps*Rstar*Dstar)+xi*(Z+lambda*Q)*K;
D=(-N+(sigma1+xi)*(Z+lambda*Q)*Kb-cryp- sigma1*eps*Rstar*Dstar)/(sigma1*R/Pi); 
...
fprintf('\nPhi = %10.12f;\n',0);
R_dc = 1;
F = @(x) [((C)^(rho_dc))*mi*x(4)*x(1)^(-rho_dc) - chi_dc*(2*(Gamma - x(2))*(x(2)/(x(1)+0) - 1/(x(1)+0))) - x(3) + betta*x(3)*ep_m/Pi;
          x(2) - x(1)/(x(1)+0);
          x(3) + x(4) - 1/(C);%/(C-zeta_0*L^(1+zeta)/(1+zeta))
          C-(mi*x(1)^(1-rho_dc)+midc*0^(1-rho_dc))^(1/(1-rho_dc))];  
%initial value for x(2) guessed at 0.5
%  ((C - h1*C1)^(-sigma1)) - beta1*h1*((C1 - h1*C1)^(-sigma1))-x(3)-x(4);
%options = optimoptions('fsolve','FunctionTolerance',1e-12,'OptimalityTolerance',1e-12); 
F = @(x) [x(3) - x(2) + betta*x(2)*ep_m/Pi;
         1/(C)-x(2)-x(3);
          C-x(1)];  
%initial value for x(2) guessed at 0.5
options = optimoptions('fsolve','FunctionTolerance',1e-8,'OptimalityTolerance',1e-8); 
[x,fval,exitflag,output,jacobian] = fsolve(F,[1 1 0.5 ],options); 
m_ss=x(1);
Lambda_ss=x(2);
gam_ss=x(3);
DC_ss=0;trs_ss=1;
...
[x,fval,exitflag,output,jacobian] = fsolve(F,[C C 1 1],options); 
trs_ss=x(2);
m_ss=x(1);
DC_ss=0;
Lambda_ss=x(3);
