...
Y = ((1-1/eta)^(1+zeta*(alpha_K+alpha_M))*(A/(eps^alpha_M*Z^alpha_K))^(1+zeta))^(1/(zeta*(1-alpha_K-alpha_M)))/((1-alpha_K-alpha_M)*zeta0^(1/zeta)); %9
...
F = @(x) [((C)^(rho_dc))*mi*x(4)*x(1)^(-rho_dc) - chi_dc*(2*(Gamma - x(2))*(x(2)/(x(1)+0) - 1/(x(1)+0))) - x(3) + betta*x(3)*ep_m/Pi;
          x(2) - x(1)/(x(1)+0);
          x(3) + x(4) - 1/(C-zeta0*L^(1+zeta)/(1+zeta));%/(C-zeta_0*L^(1+zeta)/(1+zeta))
...
[x,fval,exitflag,output,jacobian] = fsolve(F,[1 0.5 1 1],options); 
trs_ss=x(2);
m_ss=x(1);
DC_ss=0;
Lambda_ss=x(3);
